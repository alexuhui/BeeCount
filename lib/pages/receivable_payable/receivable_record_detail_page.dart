...
import '../account/account_detail_page.dart' show receivablePaymentsProvider, accountByIdProvider, allAccountStatsProvider, allAccountsTotalStatsProvider;
import 'receivable_edit_page.dart';
import 'payment_add_dialog.dart';

class ReceivableRecordDetailPage extends ConsumerWidget {
  final db.Receivable receivable;
  final String currencyCode;

  const ReceivableRecordDetailPage({
    super.key,
    ...
    return BeeScaffold(
      ...
      body: Column(
        children: [
          PrimaryHeader(
            title: '应收款详情',
            subtitle: receivable.borrowerName,
            showBack: true,
            actions: [
              IconButton(
                icon: const Icon(Icons.edit_outlined),
                onPressed: () => _editReceivable(context, ref),
              ),
            ],
          ),
          Expanded(
  ...
  Widget _buildOverviewCard(BuildContext context, WidgetRef ref, List<db.ReceivablePayment> payments) {
    final paidAmount = payments.fold<double>(0, (sum, p) => sum + p.amount);
    final remaining = receivable.amount - paidAmount;
        ...
        child: Column(
          children: [
            Row(
              ...
              children: [
                _StatItem(label: '总额', value: receivable.amount, currencyCode: currencyCode),
                _StatItem(label: '已收', value: paidAmount, currencyCode: currencyCode, color: Colors.green),
                _StatItem(label: '待收', value: remaining, currencyCode: currencyCode, color: remaining > 0 ? Colors.orange : Colors.grey),
              ],
            ),
  ...
  Widget _buildPaymentHistory(BuildContext context, WidgetRef ref, AsyncValue<List<db.ReceivablePayment>> paymentsAsync) {
    return Column(
            ...
            if (payments.isEmpty) {
              return SectionCard(
                ...
                child: Padding(
                  ...
                  child: Center(child: Text('暂无收款记录', style: TextStyle(color: BeeTokens.textTertiary(context)))),
                ),
              );
            }
            return SectionCard(
  ...
  Widget _buildBottomBar(BuildContext context, WidgetRef ref, Color primaryColor, List<db.ReceivablePayment> payments) {
    final paidAmount = payments.fold<double>(0, (sum, p) => sum + p.amount);
    final remaining = receivable.amount - paidAmount;
            ...
            Expanded(
              child: SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: const Text('记录收款', style: TextStyle(fontWeight: FontWeight.bold)),
                ),
              ),
            ),
          ],
  ...
  void _editReceivable(BuildContext context, WidgetRef ref) async {
    final account = await ref.read(accountByIdProvider(receivable.accountId).future);
    if (account != null && context.mounted) {
      await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => ReceivableEditPage(account: account, receivable: receivable),
        ),
      );
      // 返回后刷新
  ...
  void _addPayment(BuildContext context, WidgetRef ref, double remaining) async {
    ...
    if (success == true) {
      ref.invalidate(receivablePaymentsProvider(receivable.id));
      // 触发账户页刷新
...
class _PaymentTile extends ConsumerWidget {
  final db.ReceivablePayment payment;
  final String currencyCode;
  ...
  void _deletePayment(BuildContext context, WidgetRef ref) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('确定要删除这笔收款记录吗？相关的账户交易也会被删除。'),
        actions: [
    ...
    if (confirm == true) {
      await ref.read(repositoryProvider).deleteReceivablePayment(payment.id);
      ref.invalidate(receivablePaymentsProvider(payment.receivableId));
      ref.invalidate(allAccountStatsProvider);
      ref.invalidate(allAccountsTotalStatsProvider);
    }
  }
