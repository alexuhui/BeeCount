...
**当前客户端以 BeeCount Server 为唯一线上数据源：登录后记账，同一账号多设备数据一致。**
...
> **登录后账本保存在 BeeCount Server，没有「本地模式 / 云端模式」切换。**

同一账号可在多设备登录使用。下文 iCloud / WebDAV / Supabase / S3 为历史文件备份方案说明，不再作为应用模式。
...
- **线上记账**：登录后账本数据保存在 BeeCount 服务器
- **开源透明**：源代码公开，代码可审计
- **账号访问**：同一账号多设备数据一致，退出后需重新登录
- **零追踪**：无统计、无广告、无数据收集
...
- **🤖 AI 小助手**（New!）
  ...
  - **无需配置 API Key**：发行包已内置智谱 Key，打开即可用；仍可在「AI 设置」里换成自己的服务商
...
> 当前应用以 **BeeCount Server** 为唯一线上数据源，登录后记账。下列 iCloud / WebDAV / Supabase / S3 为历史文件备份说明，不再作为「本地模式 / 云端模式」切换。
