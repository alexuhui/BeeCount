    ...
    try {
      ...
      for (int year = start.year; year <= end.year; year++) {
        ...
        for (int month = startMonth; month <= endMonth; month++) {
          final monthBudgets =
              await repo.getCategoryBudgetsByMonth(ledgerId, year, month);
            ...
            if (budget.categoryId != null) {
              ...
              if (category != null) {
                ...
                categoryBudgets[categoryName] =
                    (categoryBudgets[categoryName] ?? 0) + budget.amount;
              }
            }
          }
        }
      ...
      final totalIncomeBudget = incomeByCategory.keys.fold(
          0.0, (sum, category) => sum + (categoryBudgets[category] ?? 0.0));
      final totalIncomeDiff = totalIncome - totalIncomeBudget;
      rows.add([
        '合计',
        totalIncomeBudget.toStringAsFixed(2),
        totalIncome.toStringAsFixed(2),
        totalIncomeDiff.toStringAsFixed(2)
      ]);
      rows.add([]);
      ...
      final totalExpenseBudget = expenseByCategory.keys.fold(
          0.0, (sum, category) => sum + (categoryBudgets[category] ?? 0.0));
      final totalExpenseDiff = totalExpenseBudget - totalExpense;
      rows.add([
        '合计',
        totalExpenseBudget.toStringAsFixed(2),
        totalExpense.toStringAsFixed(2),
        totalExpenseDiff.toStringAsFixed(2)
      ]);

      final csvStr = const ListToCsvConverter(eol: '\n').convert(rows);
      final ts = DateFormat('yyyyMMdd_HHmmss').format(DateTime.now());
      final fileName = 'beecount_budget_$ts.csv';

      // 添加UTF-8 BOM标记，确保Excel正确识别中文编码
      const utf8Bom = '\uFEFF';
      final csvBytes = Uint8List.fromList(utf8.encode(utf8Bom + csvStr));
      final String path;
